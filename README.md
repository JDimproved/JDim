...
JDim は GPLv2 の下で公開されている JD からforkしたソフトウェアであり、
...
##### サポートの最新情報
gccのバージョンが11未満のプラットフォームはサポートを終了しました。
Ubuntu 22.04(2022年)より前にリリースされたディストリビューションを利用されている場合は更新をお願いいたします。
...
# ↑エディタは何でも良い。deb-src行でbookworm以降を有効にする
sudo apt update
...
最近の5chサーバーの仕様変更により、JDimの旧バージョンでは書き込みができない問題が発生しています。これは主に以下の2つの原因によるものです。

1.  **古いUser-Agent文字列の拒否**: JDimのデフォルトUser-Agent (`Monazilla/1.00 JD/2006.1.1`) がサーバー側で拒否されるようになりました。
2.  **`time`パラメータの不適切さ**: 投稿時に送信される`time`パラメータの値がサーバーの期待する形式と異なっていました。

これらの問題はバージョン0.14.0で修正されます。しかし、**既存のJDimユーザーの環境設定は自動で更新されません**。そのため、バージョンアップ後も書き込みができない場合は、手動で以下の設定を変更してください。

**対応策（手動でのUser-Agent変更）:**
JDimの**about:config**を開き、「**2chにアクセスするときのエージェント名**」の値を、新しいUser-Agent文字列（例：`Monazilla/1.00 JDim/<最新バージョン> Linux x86_64`）に変更してください。

なお、IPアドレスによるBBx規制（5chサーバー側のIP制限）など、JDimのソフトウェア側では回避できない投稿規制も存在します。この場合は、ネットワーク設定の「IPv6使用(I)」の有効・無効を切り替えて（メニューバーの「設定(C)」→「ネットワーク(N)」→「IPv6使用(I)」をチェック）、書き込みを試してみてください。
...
Copyright © 2017-2019 yama-natuki [https://github.com/yama-natuki/JD]  
Copyright © 2019-2026 JDimproved project [https://github.com/JDimproved/JDim]

パッチやファイルを取り込んだ場合、それらのコピーライトは「JDimproved project」に統一します。

fork元の JD:

Copyright © 2006-2015 JD project [https://ja.osdn.net/projects/jd4linux/] \(リンク切れ\)

...
// unpack = true の時取り除く
void JDHPaned::add_remove1( bool unpack, Gtk::Widget& child )
{
...
// unpack = true の時取り除く
void JDHPaned::add_remove2( bool unpack, Gtk::Widget& child )
{
    if( unpack ) remove( child );
    else add2( child );
}
...
{
    if( m_mode == HPANED_MODE_NORMAL ) return Gtk::HPaned::get_position();
...
{
    if( m_mode == HPANED_MODE_NORMAL ) return;

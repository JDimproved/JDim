...
void ARTICLE::get_width_of_char( const char* utfstr, int& byte, const char pre_char, int& width, int& width_wide, const int mode )
{
    ...
    if( byte ){
        ...
        if( byte == 1 && ! width_of_char[ mode ][ ucs2 ].width ){
        ...
        if( byte == 1 && pre_char_num < 128 ) width = width_of_char[ mode ][ ucs2 ].width[ pre_char_num ];
        else  width = width_of_char[ mode ][ ucs2 ].width_wide;
...
{    
    ...
    // 半角モードの幅
    const int pre_char_num = ( const int ) pre_char;
    if( byte == 1 && pre_char_num < 128 ) width_of_char[ mode ][ ucs2 ].width[ pre_char_num ] = width;
